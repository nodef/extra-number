...
//#region FRACTIONAL
/**
 * Make a number purely fractional, by converting integer part to fraction.
 * @param x a number
 * @param n maximum number of digits to keep [-1: all]
 * @returns fractional number
 */
export function fractionize(x: number, n: number = -1): number {
  const xp = Math.abs(x);
  const xs = Math.sign(x);
  const xdig = xp < 1? 0 : 1 + Math.floor(Math.log10(xp));
  if (n < 0) return xs * xp * Math.pow(10, -xdig);
  const xcut = Math.max(xdig - Math.floor(n), 0);
  return Math.round(x * Math.pow(10, -xcut)) * Math.pow(10, -xdig + xcut);
}


/**
 * Convert fractional number to integer by shifting decimal point.
 * @param x a number
 * @param n maximum number of digits to shift [20]
